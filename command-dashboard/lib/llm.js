...
export function providerOrder({ search = false, force = '' } = {}) {
  const avail = availableProviders();
  if (!avail.length) return [];
  // A per-request choice (e.g. the dashboard's Claude/Gemini toggle) wins over
  // the env default — but only if that provider actually has a key configured.
  let forced = (force || '').toLowerCase().trim();
  forced = ALIAS[forced] || forced;
  const pref = (KNOWN.includes(forced) && avail.includes(forced)) ? forced : explicitPref();
  let order;
  if (search) {
...
export async function runLLM({ system = '', messages, user, maxTokens = 2000, search = false, timeoutMs = 55000, provider: force = '' }) {
  const msgs = messages || [{ role: 'user', content: user || '' }];
  const order = providerOrder({ search, force });
