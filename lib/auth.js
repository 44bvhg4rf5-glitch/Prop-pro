...
// Short-lived token issued after a correct password while 2FA is still pending.
export async function newPendingToken(accountId) { await ensureSecret(true); return signToken({ sub: accountId, exp: Date.now() + 5 * 60 * 1000, stage: '2fa' }); }
// Verify any token, recovering from a stale cached secret.
export async function readToken(token) {
  await ensureSecret(false);
  let p = verifyToken(token);
  if (!p && token) { await reloadSecret(); p = verifyToken(token); }
  return p;
}
...
// ── Password-reset tokens (single-use, 1h, hashed at rest) ──
const RESET_PREFIX = 'auth:reset:';
const THROTTLE_PREFIX = 'auth:resetlast:';
function sha(s) { return crypto.createHash('sha256').update(String(s)).digest('hex'); }
export async function createResetToken(accountId) {
  const token = crypto.randomBytes(32).toString('hex');
  await setJSON(RESET_PREFIX + sha(token), { id: accountId, exp: Date.now() + 3600 * 1000 });
  return token;
}
export async function consumeResetToken(token) {
  if (!token) return null;
  const k = RESET_PREFIX + sha(token);
  const rec = await getJSON(k, null);
  if (!rec || !rec.id || (rec.exp && Date.now() > rec.exp)) return null;
  await setJSON(k, null); // single-use
  return rec.id;
}
// Light throttle so the same address can't be spammed with reset emails.
export async function resetThrottled(email, seconds = 60) {
  const k = THROTTLE_PREFIX + sha(String(email).toLowerCase());
  const last = await getJSON(k, 0);
  if (last && Date.now() - last < seconds * 1000) return true;
  await setJSON(k, Date.now());
  return false;
}
