...
// Add many entries in a single read + write (for bulk paste). De-dupes against
// the existing list and within the batch itself.
export async function addEntries(list) {
  const { configured, entries } = await getBlocklist();
  if (!configured) return { configured: false, entries: [], added: 0 };
  const m = buildMatcher(entries);
  let added = 0;
  for (const e of list || []) {
    const probe = { uprn: e.uprn, fullAddress: e.fullAddress, postcode: e.postcode, line1: e.house || (e.fullAddress || '').split(',')[0] };
    if (isSuppressed(probe, m)) continue;
    entries.push(e);
    if (e.uprn) m.uprns.add(String(e.uprn));
    if (e.fullAddress) m.fulls.add(normAddr(e.fullAddress));
    if (e.postcode && e.house) m.pcHouse.push({ pc: (e.postcode || '').toUpperCase().replace(/\s+/g, ''), house: normAddr(e.house) });
    added++;
  }
  if (added) await saveBlocklist(entries);
  return { configured: true, entries, added };
}
