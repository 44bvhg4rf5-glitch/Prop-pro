...
// Fetch the full residential address list for a postcode. Returns
...
export async function councilTaxAddresses(postcode) {
  ...
  // 3. follow the redirect(s) to the results page, carrying cookies
  let hops = 0;
  while (p.status >= 300 && p.status < 400 && p.headers.location && hops++ < 3) {
    const loc = new URL(p.headers.location, BASE).toString();
    p = await request('GET', loc, { headers: { Cookie: jar, Referer: BASE } });
  }
  if (p.error || p.status !== 200) return { error: 'results_' + (p.error || p.status), rows: [] };
  // 4. parse address links + their bands (same order on the page)
  ...
  const addrs = [...p.body.matchAll(linkRe)].map((m) => m[1].replace(/\s+/g, ' ').trim());
  const bands = [...p.body.matchAll(bandRe)].map((m) => m[1].toUpperCase());
  const seen = new Set();
  const rows = [];
  addrs.forEach((a, i) => {
    if (seen.has(a)) return; seen.add(a);
    rows.push({ address: a, band: bands[i] || null, ...parseCouncilTax(a) });
  });
  _mem.set(mk, rows);
