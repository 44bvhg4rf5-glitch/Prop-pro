...
// Free named-landlord lookup from council HMO licence registers (currently
// Brent, HA postcodes — the statutory s.232 public register). The licence holder
// is the landlord. Published for licensing transparency, NOT marketing — callers
// should keep mail property-addressed and confirm a lawful basis before naming.
...
export function hmoLookup(line1, postcode) {
  ...
  for (const r of cands) { if (num && numOf(r.a) === num) return { ...r, council: 'Brent' }; }
  for (const r of cands) { const ra = norm(r.a); if (hay && (ra.includes(hay) || hay.includes(ra.replace(/\d+[a-z]?\s*/, '').split(',')[0].trim()))) return { ...r, council: 'Brent' }; }
  return null;
}

export function hmoByDistrict(area) {
  const a = (area || '').toUpperCase();
  return HMO_HA.filter((r) => pcNorm(r.p).startsWith(a)).map((r) => ({ ...r, council: 'Brent' }));
}
