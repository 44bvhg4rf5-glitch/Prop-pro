...
// Parse the vision JSON, tolerating truncation: try a clean parse, then closing
// an unterminated object, then regex-extracting the key fields as a last resort
// (so a cut-off reply still yields the house number it read).
function parseVision(text) {
  const clean = extractJson(text);
  if (clean) return clean;
  let t = String(text || '').replace(/```json|```/g, '').trim();
  const i = t.indexOf('{'); if (i >= 0) t = t.slice(i);
  for (const tail of ['', '"}', '}', '"]}', ']}', '"}]}']) { try { return JSON.parse(t + tail); } catch { /* next */ } }
  const g = (k) => (t.match(new RegExp('"' + k + '"\\s*:\\s*"([^"]*)"')) || [])[1] || '';
  const num = g('number'), bn = g('buildingName'), type = g('type');
  return (num || bn || type) ? { number: num, buildingName: bn, type } : {};
}
...
export async function photoAddress({ url = '', images = [], postcode = '', epcKey = '' } = {}) {
  ...
  const v = await runVision({ system: 'You are a meticulous property surveyor. Report only what is clearly visible; never invent a number.', prompt: VISION_PROMPT, images: imgs, maxTokens: 1600 });
  if (v.error) return { error: 'vision_' + v.error, note: 'The image reader is unavailable (no free vision key, or quota reached).' };
  const vision = parseVision(v.text);
  ...
  return {
    ...
    vision, visionText: (v.text || '').slice(0, 240), candidates: candidates.length, how, confidence,
