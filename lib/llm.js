...
async function geminiVision({ system, prompt, images, maxTokens, timeoutMs }) {
  ...
  // Try the proven, env-configured text model first (Flash models are multimodal),
  // then well-known vision-capable fallbacks. All Flash variants accept images.
  const models = [process.env.GEMINI_VISION_MODEL, process.env.GEMINI_MODEL, 'gemini-2.0-flash', 'gemini-flash-latest', 'gemini-1.5-flash', 'gemini-1.5-flash-latest', 'gemini-2.0-flash-lite'].filter(Boolean).filter((m, i, a) => a.indexOf(m) === i);
  ...
  for (const model of models) {
    for (const ver of ['v1beta', 'v1']) {
      const r = await postJson('https://generativelanguage.googleapis.com/' + ver + '/models/' + model + ':generateContent', { 'x-goog-api-key': key }, base, timeoutMs);
      if (r.error) { lastErr = r.error; continue; }
      if (r.status === 429) { lastErr = 'quota'; break; }
      if (r.status === 404) { lastErr = 'http_404:' + model; continue; } // model not on this version — try next
      if (!r.json || (r.status && r.status >= 400)) { lastErr = 'http_' + (r.status || '0') + ':' + model; continue; }
      const cand = (r.json.candidates || [])[0];
