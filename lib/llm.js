...
async function openaiCompatible(p, { system, messages, maxTokens, timeoutMs }) {
  ...
  // Ride out a transient rate limit (e.g. two agents firing at once) with one wait.
  if (r.status === 429) { await new Promise((s) => setTimeout(s, 3000)); r = await postJson(url, { Authorization: 'Bearer ' + key }, body, timeoutMs); }
  if (r.error) return { error: r.error };
  if (!r.json || (r.status && r.status >= 400)) return { error: 'http_' + (r.status || '0') };
