...
export async function findOwner(line1In, postcodeIn, opts = {}) {
  ...
  // 0. HMO licence register — the licence holder IS the landlord (free, named).
  try {
    const h = hmoLookup(line1, postcode);
    if (h && h.h) { owners.push({ name: h.h, role: 'landlord (HMO licence holder)', source: 'HMO register', detail: h.council + ' · ' + h.t }); sources.push('HMO register'); }
  } catch { /* ignore */ }
