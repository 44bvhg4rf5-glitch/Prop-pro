...
export async function findOwner(line1In, postcodeIn, opts = {}) {
  ...
  // 0. PRS licence register (selective + HMO) — the licence holder IS the
  // landlord (free, named, statutory public register).
  try {
    const h = hmoLookup(line1, postcode);
    if (h && h.h) { owners.push({ name: h.h, role: 'landlord (' + (h.label || 'licence') + ')', source: 'Licence register', detail: h.council + ' · ' + h.t }); sources.push('Licence register'); }
  } catch { /* ignore */ }
