...
// Raw EPC search rows for a postcode — cached in KV (searches are heavily
// rate-limited, so we never re-fetch a postcode we've seen), with 429 backoff.
const sleep = (ms) => new Promise((s) => setTimeout(s, ms));
const _searchMem = new Map();
async function epcSearch(pc, key) {
  const ck = 'epcs:' + String(pc).toUpperCase().replace(/\s+/g, '');
  if (_searchMem.has(ck)) return _searchMem.get(ck);
  if (storeConfigured()) { const c = await getJSON(ck, null); if (Array.isArray(c)) { _searchMem.set(ck, c); return c; } }
  const url = `${EPC_BASE}/api/domestic/search?postcode=${encodeURIComponent(pc).replace(/%20/g, '+')}&page_size=500`;
  let data = [];
  for (const wait of [0, 700, 1600, 3200]) {
    if (wait) await sleep(wait);
    const { status, json } = await fetchJson(url, key);
    if (status === 200 && json && Array.isArray(json.data)) { data = json.data; break; }
    if (status !== 429) break; // 401/403/other → give up (don't burn retries)
  }
  if (data.length) { _searchMem.set(ck, data); if (_searchMem.size > 3000) _searchMem.clear(); if (storeConfigured()) setJSON(ck, data).catch(() => {}); }
  return data;
}
...
async function epcRows(pc, key) {
  if (!key) return [];
  try {
    const data = await epcSearch(pc, key);
...
async function certTenure(cert, key) {
  ...
  const ck = 'epct:' + cert;
  if (storeConfigured()) { const c = await getJSON(ck, null); if (c) { _certTenure.set(cert, c); return c; } }
  let t = 'unknown';
  ...
  // Persist only decided tenures — an "unknown" may just be a throttled read.
  if (storeConfigured() && t !== 'unknown') setJSON(ck, t).catch(() => {});
  return t;
}
...
async function tenureSequential(rows, key, gap, expired) {
  const out = [];
  for (let i = 0; i < rows.length; i++) {
    if (expired && expired()) { for (let j = i; j < rows.length; j++) out[j] = 'unknown'; break; }
...
export async function rentedHomesForArea(postcodes, key, { start = 0, certBudget = 170, gap = 110, deadlineMs = 50000 } = {}) {
  const t0 = Date.now();
  const all = [];
  let scanned = 0, epcHomes = 0, withTenure = 0, certs = 0, i = start, timedOut = false;
  for (; i < postcodes.length && certs < certBudget; i++) {
    if (Date.now() - t0 > deadlineMs) { timedOut = true; break; } // return partial, never 504
    ...
    const tenures = await tenureSequential(picks, key, gap, () => Date.now() - t0 > deadlineMs);
    certs += picks.length;
    const { rented, withTenure: wt } = collectRented(picks, tenures);
    withTenure += wt; all.push(...rented);
  }
  return { postcodesScanned: scanned, postcodesAvailable: postcodes.length, nextStart: i, done: !timedOut && i >= postcodes.length, timedOut, certsRead: certs, epcHomes, withTenure, rented: all };
}
