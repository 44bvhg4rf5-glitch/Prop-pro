...
// ── Rightmove ──
export async function rightmoveListings(district, { channel = 'sale', minBeds = 0, maxPrice = 0, index = 0, pages = 1 } = {}) {
  const outcode = OUTCODES[district];
  if (!outcode) return [];
  const seg = channel === 'rent' || channel === 'let' ? 'property-to-rent' : 'property-for-sale';
  // Fetch the requested number of result pages (24 listings each) in parallel.
  const indexes = pages > 1 ? Array.from({ length: pages }, (_, i) => i * 24) : [index];
  const pageResults = await Promise.all(indexes.map((idx) => {
    const q = new URLSearchParams({ locationIdentifier: `OUTCODE^${outcode}`, index: String(idx), includeSSTC: 'false' });
