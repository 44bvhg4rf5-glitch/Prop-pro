...
export async function streetIntel({ streetName = '', postcodes = [], homes = 0, outcode = '', lrBudget = 12 }) {
  const oc = (outcode || (postcodes[0] || '').toUpperCase().match(/^[A-Z]{1,2}\d[A-Z\d]?/)?.[0] || '').toUpperCase();
  const lic = licensedRentals(streetName, postcodes, oc);
  const sold = await recentSales(streetName, postcodes, { lrBudget, sinceISO: fiveYearsAgoISO() });
  ...
  // Targeting steer — licensed lets are a FLOOR (only licensable lets show), so
  // any meaningful count signals a rental street; otherwise lean on sales churn.
  let verdict, focus;
  if (lic.length >= 5) { verdict = 'Strong licensed-rental presence — a prime street for landlord letters.'; focus = 'landlord'; }
  else if (lic.length >= 1) { verdict = 'Licensed lettings here — worth a mix of landlord and vendor letters.'; focus = 'mixed'; }
  ...
  return {
    homes, licensedRentals: lic.length, rentalCompanies: companies, sold5y: sold, soldPct, focus, verdict,
    lines: [
      homes ? `${homes} homes here` : null,
      lic.length ? `${lic.length} licensed rental${lic.length === 1 ? '' : 's'} on this street${companies ? ` (${companies} via companies)` : ''}` : 'no licensed rentals on record',
