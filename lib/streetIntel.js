...
export async function streetIntel({ streetName = '', postcodes = [], homes = 0, outcode = '', epcKey = '', lrBudget = 12 }) {
  ...
  const [lic, sold, epc] = await Promise.all([
    ...
    rentalStatsForStreet(streetName, postcodes, epcKey).catch(() => ({ withTenure: 0, privateRented: 0, rentalRate: null, rentedAddresses: [] })),
  ]);
  ...
  // EPC tenure (cert-sampled) gives a borough-wide private-rental RATE — works
  // where the licence register doesn't (HA1/2/3).
  const epcSample = epc.withTenure || 0;
  const rentalRate = epc.rentalRate; // null unless the tenure sample was big enough
