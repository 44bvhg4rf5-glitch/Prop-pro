...
function recOf(p, prev, nowISO) {
  return {
    ...
    beds: p.beds || 0,
    propType: p.type || '',
    channel: p.channel || (prev && prev.channel) || 'sale',
    lat: p.lat != null ? p.lat : (prev ? prev.lat : null),
    lon: p.lon != null ? p.lon : (prev ? prev.lon : null),
  };
}

// How did a listing leave the market? From its last live status + channel.
function exitReason(was) {
  if (isAgreed(was.liveStatus)) return 'sold';                 // Sold STC / Under Offer → completed
  if (/let agreed/i.test(was.liveStatus || '')) return 'let';
  if (was.channel === 'rent') return 'let';                    // a rental that came off is almost always let
  return 'withdrawn';                                          // an available sale that vanished = vendor pulled it
}
const daysISO = (aISO, bISO) => { const a = Date.parse(aISO), b = Date.parse(bISO); return (Number.isNaN(a) || Number.isNaN(b)) ? null : Math.round(Math.abs(a - b) / 86400000); };
...
export function classify(prevSnap, today, nowISO) {
  ...
  for (const id in prev) {
    ...
    // Confirm a disappearance over 2 scans so a single failed fetch can't invent
    // an exit. At that point record it in the off-market database, classified by
    // how it left, and (for sale withdrawals) raise a touting lead.
    if (!firstRun && misses === 2) {
      const reason = exitReason(was);
      if (reason === 'withdrawn') events.push(mkEvent('withdrawn', was, nowISO));
      offMarket.push({
        id: was.id, addr: was.addr, postcode: was.postcode, district: was.district,
        propType: was.propType, beds: was.beds, price: was.price, agent: was.agent,
        source: was.source, channel: was.channel || 'sale', url: was.url,
        lat: was.lat, lon: was.lon, firstListed: was.firstListed,
        firstSeen: was.firstSeen, lastSeen: was.lastSeen, offDate: nowISO,
        reason, dom: daysISO(was.firstListed || was.firstSeen, nowISO),
      });
    }
    if (misses < 4) snapshot[id] = { ...was, misses };   // keep briefly to absorb a quick relist / fetch gap
  }

  return { snapshot, events, offMarket };
}
